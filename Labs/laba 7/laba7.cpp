...
int main() {
	...
	try {
		...
		DynamicList<string> stringList{ "один", "два", "четыре", "восемь", "шестнадцать" };
		...
		// Проверка обхода положительных элементов
		cout << "Положительные элементы List: ";
		auto positiveIt = intList.positiveIterator();
		while (positiveIt.hasNext()) {
			cout << positiveIt.next() << " ";
		}
		cout << endl;

		// Проверка обхода отрицательных элементов
		cout << "Отрицательные элементы List: ";
		auto negativeIt = intList.negativeIterator();
		while (negativeIt.hasNext()) {
			cout << negativeIt.next() << " ";
		}
		cout << endl;
